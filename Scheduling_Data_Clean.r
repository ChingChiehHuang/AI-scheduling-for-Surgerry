...
#delete patient with two or more columns at the same days
...
###############   Num_of_OpType2131  ###############
Train$Num_of_OpType2131 <-
foreach(i = 1:dim(Train)[1], .combine=c) %dopar% {
  begin <- Train$exe_times[i]
  Dr_group <- Train[Train$DrID %in% Train$DrID[i],]
  sum(Dr_group$exe_times >= begin-days(1) &
        Dr_group$exe_times <= begin-1 & Dr_group$OpType %in% c('21','31') ,na.rm = TRUE)
}

Test$Num_of_OpType2131 <- 
  foreach(i = 1:dim(Test)[1], .combine=c) %dopar% {
  begin <- Test$exe_times[i]
...
#delete doctor age <28
Train <- Train[Train$Dr_age >=28,];dim(Train)
Test <- Test[Test$Dr_age >=28,];dim(Test)
...
#in_mins 
Train$OUT %<>% hm();table(is.na(Train$OUT))
Train$In %<>% hm();table(is.na(Train$In))
Test$OUT %<>% hm();table(is.na(Test$OUT))
Test$In %<>% hm();table(is.na(Test$In))
...
### The number of procedure and total duration
r1 <- 
...
#delete procedure duration > 10 hrs or < 10
...
# Delete all procedure were missing value
...
# Delete all diagnosis were missing value
...
## Doctors
Doctor <- data.frame(Key, Train$DrID, Train$DrType2, Train$DrType3, Train$DrType4, Train$DrType5)
colnames(Doctor) <- c('Key','Dr1','Dr2','Dr3','Dr4','Dr5')

DOC <- melt(Doctor ,id.var="Key"); dim(DOC)
DOC_name <- names(table(DOC$value))[table(DOC$value) > 500]
DOC_name

Train$DrID[!(Train$DrID %in% DOC_name) & !is.na(Train$DrID)] <- 'Other_doc'
Train$DrType2[!(Train$DrType2 %in% DOC_name) & !is.na(Train$DrType2)] <- 'Other_doc'
Train$DrType3[!(Train$DrType3 %in% DOC_name) & !is.na(Train$DrType3)] <- 'Other_doc'
Train$DrType4[!(Train$DrType4 %in% DOC_name) & !is.na(Train$DrType4)] <- 'Other_doc'
Train$DrType5[!(Train$DrType5 %in% DOC_name) & !is.na(Train$DrType5)] <- 'Other_doc'

Test$DrID[!(Test$DrID %in% DOC_name) & !is.na(Test$DrID)] <- 'Other_doc'
Test$DrType2[!(Test$DrType2 %in% DOC_name) & !is.na(Test$DrType2)] <- 'Other_doc'
Test$DrType3[!(Test$DrType3 %in% DOC_name) & !is.na(Test$DrType3)] <- 'Other_doc'
Test$DrType4[!(Test$DrType4 %in% DOC_name) & !is.na(Test$DrType4)] <- 'Other_doc'
Test$DrType5[!(Test$DrType5 %in% DOC_name) & !is.na(Test$DrType5)] <- 'Other_doc'
...
#Viewed unusual used operating room (< 500) as others
Train$OpRoom[Train$OpRoom %in%  names(table(Train$OpRoom))[table(Train$OpRoom) < 500]] <- 'Others'
Test$OpRoom[!(Test$OpRoom %in% Train$OpRoom)] <- 'Others'

#Viewed unusual DivNo (< 20) as others
...
#Train$Anemia[is.na(Train$Anemia)] <- 'unknown'
#Train$Diabetes[is.na(Train$Diabetes)] <- 'unknown'
Train$Hypertension[is.na(Train$Hypertension)] <- 'unknown'
Train$COPD[is.na(Train$COPD)] <- 'unknown'
Train$CKD[is.na(Train$CKD)] <- 'unknown'


Test$ASA[is.na(Test$ASA)] <- 'unknown'
#Test$Anemia[is.na(Test$Anemia)] <- 'unknown'
#Test$Diabetes[is.na(Test$Diabetes)] <- 'unknown'
Test$Hypertension[is.na(Test$Hypertension)] <- 'unknown'
Test$COPD[is.na(Test$COPD)] <- 'unknown'
Test$CKD[is.na(Test$CKD)] <- 'unknown'


## AnaDr_year
Train$AnaDr_year <- ifelse(Train$AnaDr_year >= 10,'over_10','less_10')
Train$AnaDr_year[is.na(Train$AnaDr_year)] <- 'unknown'

Test$AnaDr_year <- ifelse(Test$AnaDr_year >= 10,'over_10','less_10')
Test$AnaDr_year[is.na(Test$AnaDr_year)] <- 'unknown'
...
Train$COPD %<>% as.factor()
Train$CKD %<>% as.factor()
Train$OpRoom %<>% as.factor()
...
Train$DrID %<>% as.factor()
Train$DrType2 %<>% as.factor()
Train$DrType3 %<>% as.factor()
Train$DrType4 %<>% as.factor()
Train$DrType5 %<>% as.factor()
Train$AnaDr_year %<>% as.factor()
Train$BMI %<>% as.factor()
...
Test$DrID %<>% as.factor()
Test$DrType2 %<>% as.factor()
Test$DrType3 %<>% as.factor()
Test$DrType4 %<>% as.factor()
Test$DrType5 %<>% as.factor()
Test$AnaDr_year %<>% as.factor()
Test$BMI %<>% as.factor()

#########################################################

Train %<>% select(-Weight,-Height,-In,-OUT,-exe_times,-Dr_year)
Test %<>% select(-Weight,-Height,-In,-OUT,-exe_times,-Dr_year)
